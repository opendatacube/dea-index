...
until docker logs docker_index | grep -m 1 "Updating range for all, using SQL extent calculation"; do sleep 10; done

# Save the image with built database
#docker stop docker_postgres
docker commit docker_postgres opendatacube/dea-index:$(date +%Y-%m-%d)
